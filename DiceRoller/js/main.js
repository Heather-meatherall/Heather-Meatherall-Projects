...
function roll(tableID) {
  ...
  for (let i = 0; i < amount; i++) {
    //using the rollDice function, generate a number between 1 and the max value of the chosen di
    const di = rollDice(sides);
    dice = dice + di;
    diceRolls.push(di);
  }
...
function deathSaves(tableID, diceElementId, buttonElementId) {
  ...
  // death_button.addEventListener("click",onClick);

  //using the rollDice function, generate a number between 1 and 20
  const di = rollDice(20);
  ...
  if (di == 20)
  {
    ...
    death_dice.value = "Nat 20!"
    death_button.disabled = true; // prevents the button from being rolled more then once
  }
  else if (di == 1)
  {
